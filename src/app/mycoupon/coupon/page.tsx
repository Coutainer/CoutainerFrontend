  ...
  async function handleSellSubmit() {
    ...
    try {
      ...
      const res = await fetch("/api/marketplace/list-for-sale", {
        ...
        body: JSON.stringify({ objectId: item.objectId, price: trimmed }), // 백엔드 스펙: string
      });
      ...
      // 필요 시 페이지 이동
      // router.push("/market");  // 혹은 판매목록 페이지로 이동
      setSellOpen(false);
    } catch (e: any) {
        ...
        <div className="bg-white rounded-2xl shadow-lg w-[320px] p-6 flex flex-col items-center mt-0 mx-auto mb-20">
          ...
          {!pending && !error && item && (
            <>
              ...
              <button
                onClick={() => alert(`쿠폰 사용하기: ${item.objectId}`)}
        ...
        {sellOpen && (
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center p-6">
            <div className="w-full max-w-sm bg-white rounded-2xl p-5 shadow-xl">
              ...
              {submitOk && (
                <p className="text-sm text-emerald-600 mt-2">{submitOk}</p>
              )}
